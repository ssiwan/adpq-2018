...
function updateTaskDefinition() {
    ...
    sed -e "s;%ECS_FAMILY%;${ECS_FAMILY};g" -e "s;%BUILD_NUMBER%;${BUILD_NUMBER};g" -e "s;%REPOSITORY_URI_SERVER%;${REPOSITORY_URI_SERVER};g" -e "s;%REPOSITORY_URI_WEB%;${REPOSITORY_URI_WEB};g" -e "s;%ENVIRONMENT%;${ENVIRONMENT};g" ./taskdef.json > ${ECS_TASK_DEFINITION}-v_${BUILD_NUMBER}.json

    echo ${ECS_TASK_DEFINITION}
    echo ${BUILD_NUMBER}
    echo ${ECS_TASK_DEFINITION}-v_${BUILD_NUMBER}
